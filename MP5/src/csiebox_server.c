...
void my_handler(int signum){
  if (signum == SIGUSR1){
      int fd = open(mServer->fifo, O_WRONLY);
      // printf("Received SIGUSR1! active:%d, fifo:%d\n", active_thread_num, mServer->fifo_fd);
      uint32_t threadNum = htonl(active_thread_num);
      fprintf(logFile, "Current active thread %d", threadNum);
      fflush(logFile);
      write(fd, &threadNum, sizeof(threadNum));
...
void terminate_handler_SIGTERM(int signum){
   if(signum == SIGTERM){
...
void daemonize(){
  ...
  // PARENT PROCESS. Need to kill it.
    ...
    fprintf(logFile, "process_id of child process %d \n", process_id);
    // return success in exit status
    exit(0);
  }
  if(process_id == 0){
    fprintf(logFile, "I am child process :(\n", process_id);
  }
  fprintf(logFile, "ready tp unmask!\n");
  //unmask the file mode
  umask(0);
  //set new session
  fprintf(logFile, "Set new session!\n");
  sid = setsid();
  ...
  int dfd = open("dump_output.txt", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
  dup2(dfd, 1);   // make stdout go to file
  dup2(dfd, 2);   // make stderr go to file - you may choose to not do this
  // close(0);
  // close(1);
  // close(2);
...
void csiebox_server_init(csiebox_server** server, int argc, char** argv) {
  ...
  *server = tmp;
  ...
  fprintf(stderr, (*server)->arg.run_path);
  chdir((*server)->arg.run_path);
  logFile = fopen("log.txt", "w");
  setvbuf ( logFile , NULL , _IONBF , 1);
  fprintf(logFile, "read to daemonize\n");

  fprintf(stderr, "%d \n", (*server)->arg.daemonize);
  if((*server)->arg.daemonize){
    ...
    chdir((*server)->arg.run_path);
    FILE* file = fopen("csiebox_server.pid", "w");
    setvbuf ( file , NULL , _IONBF , 1);
    fprintf(logFile, "Write pid to file %s\n", pidStr);
    fprintf(file, "%s", pidStr);
    strcpy((*server)->fifo, "fifo.");
    strcat((*server)->fifo, pidStr);
  }
  else{
    // read pid from file
    FILE* file = fopen("csiebox_server.pid", "r");
    char pid_buffer[1024];
    int result = fread(pid_buffer,1,512,file);
    pid_buffer[strcspn(pid_buffer, "\n")] = 0;
    fprintf(logFile, "pid: %s\n", pid_buffer);
    strcpy((*server)->fifo, "fifo.");
    strcat((*server)->fifo, pid_buffer);
    fprintf(logFile,"%s\n", mServer->fifo);
  }
  ...
  signal(SIGTERM, terminate_handler_SIGTERM);

  
  
  
  // int csiebox_server_pid_fd = open("csiebox_server.pid", "w");
  // fprintf(csiebox_server_pid_fd,"%d", 1234567);
 

  int ret;
  ret = mkfifo((*server)->fifo, 0666);
  fflush(logFile);
  // mServer->fifo_fd = -1;
  // mServer->fifo_fd = open(mServer->fifo, O_WRONLY);
